...
from Whiskey import YouTube, app
from Whiskey.core.userbot import assistants
from Whiskey.misc import SUDOERS, pymongodb
from Whiskey.plugins import ALL_MODULES
from Whiskey.utils.database import (get_global_tops,
                                       get_particulars, get_queries,
                                       get_served_chats,
                                       get_served_users, get_sudoers,
                                       get_top_chats, get_topp_users)
from Whiskey.utils.decorators.language import language, languageCB
from Whiskey.utils.inline.stats import (back_stats_buttons,
                                           back_stats_markup,
                                           get_stats_markup,
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"🍒 [𓏺˛ 𝙒𝙃𝙄𝙎𝙆𝙀𝙔 𝙎𝙊𝙐𝙍𝘾𝙀 .](https://t.me/HL_BG) ** التشفيل {count} الوقت**\n\n"
                else:
...
**الوحد القروبات:** {mod}
** القروبات والقنو القروبات:** {served_chats} 
**المستخدمين:** {served_users} 
**المحظورين:** {blocked} 
**المطورين:** {sudoers} 
    
**الاستفسار القروبات:** {total_queries} 
**المساعدين:** {assistant}
**التوقف القروبات:** {ass}
...
**ғʀᴇᴇ:** {free[:4]} GiB
      ...
      <b><u>الاحصائي القروبات العامه</b><u/>
...
**المف القروباتيح:** {objects}
**الاستفسار القروبات:** `{query}`
**استفسار القروبات البوت:** `{total_queries} `
    """
    med = InputMediaPhoto(media=config.STATS_IMG_URL, caption=text)
    try:
        await CallbackQuery.edit_message_media(
